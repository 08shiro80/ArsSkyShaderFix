...
public final class SkyFallbackRenderer {
    ...
    private static final ResourceLocation WHITE = ResourceLocation.fromNamespaceAndPath("arsskyfix", "textures/white.png");
    ...
    private static void emitFace(int f, VertexConsumer vc, Matrix4f matrix, float[] base) {
        ...
        for (int v = 0; v < 4; v++) {
            vc.addVertex(matrix, c[v][0] + n[0] * CubeFaces.EPS, c[v][1] + n[1] * CubeFaces.EPS, c[v][2] + n[2] * CubeFaces.EPS)
                .setColor(r, g, bl, 1.0f)
                .setUv(UVS[v][0], UVS[v][1])
